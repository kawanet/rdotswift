...
const Identifier = require("./src/ATS/Identifier");
const TokenKinds = require("./src/Syntax/TokenKinds");

const CLASS = "class";
const IF = "if";
const TYPES = ["array", "bool", "color", "dimen", "integer", "string"];
...
function format(R, options) {
  ...
  if (!options.extension) {
    const className = options[CLASS] || "R";
  ...
  function colorFilter(key, val) {
    ...
    if (val.match(/^#[0-9A-Fa-f]{6,8}$/)) {
      let rgb = parseInt(val.substr(1), 16);
      const blue = rgb & 0xFF;
      rgb >>= 8;
      const green = rgb & 0xFF;
      rgb >>= 8;
      const red = rgb & 0xFF;
      rgb >>= 8;
      const alpha = (val.length === 9) ? (rgb & 0xFF) : 255;

      const uicolor = options.appkit ? "NSColor" : "UIColor";
...
function makeExtension(type, src, filter, options) {
  ...
  if (src) Object.keys(src).forEach(function(key) {
    ...
    const row = filter(key, val);
    if (!row) return;

    const comment = raw && raw.comment;
